...
if (!isset($_REQUEST['p'])) {
  header("location:clientes.php?p=1");
}
  ...
  <body class="app header-fixed sidebar-fixed aside-menu-fixed sidebar-lg-show">
      ...
      <div class="sidebar">
        <nav class="sidebar-nav">
          <ul class="nav">
            ...
            <br>
            <li class="nav-item">
              <a class="nav-link" href="dashboard-turismo.php?p=1" style="background: #20a8d8;">
                <i class="nav-icon icon-people" style="color: #fff;"></i> Clientes 
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="encuesta.php?p=1">
                <i class="nav-icon icon-docs"></i> Encuesta  
              </a>
            </li>
            <li class="nav-item">
              ...
              <div class="col-lg-12">
                <div class="card">
                  ...
                  <div class="card-body">
                      ...
                      <tbody>
                        <?php
                          ...
                          if (isset($_REQUEST['buscar'])) {
                            $query_cont = 'SELECT * from clientes_turismo where nombre_clientetur like "%'.$_REQUEST['buscar'].'%" OR correo_clientetur like "%'.$_REQUEST['buscar'].'%"';
                          } else {
                            $query_cont = 'SELECT * from clientes_turismo';
                          }
                          ...
                          if (isset($_REQUEST['b'])) {
                            $order = ' order by STR_TO_DATE(fecha_registro, "%d/%m/%Y") desc';
                          } else {
                            $order = '';
                          }

                          if (isset($_REQUEST['buscar'])) {
                            $query = 'SELECT * from clientes_turismo where nombre_clientetur like "%'.$_REQUEST['buscar'].'%" OR correo_clientetur like "%'.$_REQUEST['buscar'].'%"'.$order.' limit '.$inicio.', 10';
                          } else {
                            $query = 'SELECT * from clientes_turismo '.$order.' limit '.$inicio.', 10';
                          }
                          ...
                          foreach ($sql_obj as $row) { ?>
                            ...
                            <tr>
                              ...
                              <td><?php echo $row['nombre_clientetur']; ?></td>
                              <td><?php echo $row['correo_clientetur']; ?></td>
                              <td><?php echo $row['telefono_clientetur']; ?></td>
                              <td><?php echo $row['id']; ?></td>
                            </tr>
                    ...
                    <?php
                      if (isset($_REQUEST['b'])) {
                        $url_atras = 'dashboard-turismo.php?b='.$_REQUEST['b'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?b='.$_REQUEST['b'].'&p='.($_REQUEST['p']+1);
                      } elseif (isset($_REQUEST['m'])) {
                        $url_atras = 'dashboard-turismo.php?m='.$_REQUEST['m'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?m='.$_REQUEST['m'].'&p='.($_REQUEST['p']+1);
                      } elseif (isset($_REQUEST['e'])) {
                        $url_atras = 'dashboard-turismo.php?e='.$_REQUEST['e'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?e='.$_REQUEST['e'].'&p='.($_REQUEST['p']+1);
                      } elseif (isset($_REQUEST['a'])) {
                        $url_atras = 'dashboard-turismo.php?a='.$_REQUEST['a'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?a='.$_REQUEST['a'].'&p='.($_REQUEST['p']+1);
                      }  elseif (isset($_REQUEST['buscar'])) {
                        $url_atras = 'dashboard-turismo.php?buscar='.$_REQUEST['buscar'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?buscar='.$_REQUEST['buscar'].'&p='.($_REQUEST['p']+1);
                      } elseif (isset($_REQUEST['consulta'])) {
                        $url_atras = 'dashboard-turismo.php?consulta='.$_REQUEST['consulta'].'&p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?consulta='.$_REQUEST['consulta'].'&p='.($_REQUEST['p']+1);
                      } else {
                        $url_atras = 'dashboard-turismo.php?p='.($_REQUEST['p']-1);
                        $url_next = 'dashboard-turismo.php?p='.($_REQUEST['p']+1);
                      }
                    ?>
                    <?php if (isset($_REQUEST['b'])) { ?>
                      <div id="resultados-hide" class="d-none">Resultado: <b><?php echo $cont; ?></b> operativos en total ordenados por <b><?php if ($_REQUEST['b'] == 'num_operativo') { echo 'Numero de operativo'; } elseif ($_REQUEST['b'] == 'fecha_operativo') { echo 'Fecha de Registro'; } elseif ($_REQUEST['b'] == 'num_vehiculo') { echo 'Numero interno de vehiculo'; }; ?></b>.</div>
                    ...
                    <nav>
                      <ul class="pagination">
                        ...
                        <?php 
                          ...
                          while ($i <= $num_item) {
                            if (isset($_REQUEST['b'])) {
                              $url_pagina = 'dashboard-turismo.php?b='.$_REQUEST['b'].'&p='.$i;
                            } elseif (isset($_REQUEST['m'])) {
                              $url_pagina = 'dashboard-turismo.php?m='.$_REQUEST['m'].'&p='.$i;
                            } elseif (isset($_REQUEST['buscar'])) {
                              $url_pagina = 'dashboard-turismo.php?buscar='.$_REQUEST['buscar'].'&p='.$i;
                            } elseif (isset($_REQUEST['consulta'])) {
                              $url_pagina = 'dashboard-turismo.php?consulta='.$_REQUEST['consulta'].'&p='.$i;
                            }  else {
                              $url_pagina = 'dashboard-turismo.php?p='.$i;
                            }
                        ?>
    ...
    <script>
      ...
      $('#agregados').on('change', function() {  
        ...
        window.location.href="dashboard-turismo.php?a="+this.value+"&p=1";
      });
      ...
      $('#order').on('change', function() {  
        ...
        if (this.value == 'num_operativo') {
          window.location.href="dashboard-turismo.php?b=num_operativo&p=1";
        } 
        if (this.value == 'fecha_operativo') {
          window.location.href="dashboard-turismo.php?b=fecha_operativo&p=1";
        } 
        if (this.value == 'placa_vehiculo') {
          window.location.href="dashboard-turismo.php?b=placa_vehiculo&p=1";
        }
        if (this.value == 'num_vehiculo') {
          window.location.href="dashboard-turismo.php?b=num_vehiculo&p=1";
        }
        if (this.value == 'no') {
          window.location.href="dashboard-turismo.php?p=1";
        }
      });
      ...
      $('#modalidad').on('change', function() {  
        ...
        if (this.value == 'Doble Yo') {
          window.location.href="dashboard-turismo.php?m=Doble Yo&p=1";
        } 
        if (this.value == 'VIP') {
          window.location.href="dashboard-turismo.php?m=VIP&p=1";
        } 
        if (this.value == 'Platino Expres') {
          window.location.href="dashboard-turismo.php?m=Platino Expres&p=1";
        }
        if (this.value == 'Platino Jet') {
          window.location.href="dashboard-turismo.php?m=Platino Jet&p=1";
        }
        if (this.value == 'Platino Especial') {
          window.location.href="dashboard-turismo.php?m=Platino Especial&p=1";
        }
        if (this.value == 'Mixto') {
          window.location.href="dashboard-turismo.php?m=Mixto&p=1";
        }
        if (this.value == 'no') {
          window.location.href="dashboard-turismo.php?p=1";
        }
      ...
      $('#mes').on('change', function() {  
        ...
        if (this.value == 'Enero') {
          window.location.href="dashboard-turismo.php?buscar=2019-01&mes=Enero&p=1";
        } 
        if (this.value == 'Febrero') {
          window.location.href="dashboard-turismo.php?buscar=2019-02&mes=Febrero&p=1";
        }
        if (this.value == 'Marzo') {
          window.location.href="dashboard-turismo.php?buscar=2019-03&mes=Marzo&p=1";
        }
        if (this.value == 'Abril') {
          window.location.href="dashboard-turismo.php?buscar=2019-04&mes=Abril&p=1";
        }
        if (this.value == 'Mayo') {
          window.location.href="dashboard-turismo.php?buscar=2019-05&mes=Mayo&p=1";
        }
        if (this.value == 'Junio') {
          window.location.href="dashboard-turismo.php?buscar=2019-06&mes=Junio&p=1";
        }
        if (this.value == 'Julio') {
          window.location.href="dashboard-turismo.php?buscar=2019-07&mes=Julio&p=1";
        }
        if (this.value == 'Agosto') {
          window.location.href="dashboard-turismo.php?buscar=2019-08&mes=Agosto&p=1";
        }
        if (this.value == 'Septiembre') {
          window.location.href="dashboard-turismo.php?buscar=2019-09&mes=Septiembre&p=1";
        }
        if (this.value == 'Octubre') {
          window.location.href="dashboard-turismo.php?buscar=2019-10&mes=Octubre&p=1";
        }
        if (this.value == 'Noviembre') {
          window.location.href="dashboard-turismo.php?buscar=2019-11&mes=Noviembre&p=1";
        }
        if (this.value == 'Diciembre') {
          window.location.href="dashboard-turismo.php?buscar=2019-12&mes=Diciembre&p=1";
        }
        if (this.value == 'no') {
          window.location.href="dashboard-turismo.php?p=1";
        }
      ...
      $('#buscar').keyup(function(e){
          if(e.keyCode == 13)
          {
            ...
            window.location.href = 'dashboard-turismo.php?buscar='+busqueda+"&p=1";
          }
      });
      ...
      $("#enviar-avanzada").click(function() {
        ...
        window.location.href = "dashboard-turismo.php?consulta="+sql+"&p=1";
