...
$sql_op = $objConexion->prepare('SELECT * from operativos where id_operativo = :num');
...
foreach ($res_op as $row) {
	...
	<?php } else {
		$sql = $objConexion->prepare('UPDATE operativos set observaciones = :falta where id_operativo = :num');
